...
def combine_dataframes(dframes,source):
    ...
    if source == 'wunderground':
        df = pd.concat(dframes,ignore_index=True)
        # df = df.reset_index(drop=True).iloc[:,1:]
        df.drop_duplicates(subset=['stationID','obsTimeLocal','temperature','epoch'],inplace=True)
...
def process_downloaded_data(location_name,source,item_identifier,year):
    ...
    valid_files = [input_path+'/' + x for x in os.listdir(input_path) if '.csv' in x]
    master_df = combine_dataframes([pd.read_csv(x) for x in valid_files],source=source)
...
def aggregation(df):
    ...
    df = df.reset_index()
    df['beg_time'] = pd.to_datetime(df.beg_time)
    df['day_of_year'] = df['beg_time'].apply(lambda x : datetime.timetuple(x).tm_yday)
...
def process_lter(lter_combined):
    ...
    lter_combined['beg_time'] = pd.to_datetime(lter_combined.beg_time)
    lter_combined['day_of_year'] = lter_combined['beg_time'].apply(lambda x : datetime.timetuple(x).tm_yday)
    lter_combined['value'] = (lter_combined['value']-32)*(5/9)

    lter_combined.columns = ['station', 'beg_time', 'temperature', 'latitude', 'longitude', 'day_of_year']
...
def process_wunder(pws_df):
    ...
    pws_df['day_of_year'] = pws_df['beg_time'].apply(lambda x : datetime.timetuple(x).tm_yday)
